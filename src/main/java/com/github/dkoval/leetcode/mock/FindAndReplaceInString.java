...
public class FindAndReplaceInString {
    ...
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        // contains (index in S, index of replacement operation) pairs
        Map<Integer, Integer> replacement = new HashMap<>();
        for (int i = 0; i < indexes.length; i++) {
            int index = indexes[i];
            if (S.indexOf(sources[i], index) == index) {
                replacement.put(index, i);
            }
        }
        StringBuilder sb = new StringBuilder();
        int index = 0;
        while (index < S.length()) {
            if (replacement.containsKey(index)) {
                int i = replacement.get(index);
                sb.append(targets[i]);
                index += sources[i].length();
            } else {
