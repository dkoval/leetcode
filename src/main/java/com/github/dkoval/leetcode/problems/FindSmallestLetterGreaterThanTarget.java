...
public interface FindSmallestLetterGreaterThanTarget {
    ...
    // O(logN) time | O(1) space
    class FindSmallestLetterGreaterThanTargetBinarySearchRev3 implements FindSmallestLetterGreaterThanTarget {
        ...
        public char nextGreatestLetter(char[] letters, char target) {
            ...
            while (left < right) {
                ...
                if (letters[mid] > target) {
                    // mid may be the answer, check if there's a better alternative to the left
                    right = mid;
                } else {
                    left = mid + 1;
                }
