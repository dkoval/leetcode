...
public interface MinCostClimbingStairs {
    ...
    // O(N) time | O(N) space
    class MinCostClimbingStairsDPBottomUpRev3 implements MinCostClimbingStairs {

        @Override
        public int minCostClimbingStairs(int[] cost) {
            int n = cost.length;

            // dp[i] is the min cost to get to index i
            int[] dp = new int[n + 2];
            Arrays.fill(dp, Integer.MAX_VALUE);
            dp[0] = 0;
            dp[1] = 0;
    ...
    // O(N) time | O(N) space
    class MinCostClimbingStairsDPBottomUpRev4 implements MinCostClimbingStairs {
