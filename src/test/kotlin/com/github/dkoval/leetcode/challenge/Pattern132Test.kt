package com.github.dkoval.leetcode.challenge

import com.github.dkoval.leetcode.challenge.Pattern132.Pattern132BruteForceInCubicTime
import com.github.dkoval.leetcode.challenge.Pattern132.Pattern132BruteForceInQuadraticTime
import org.junit.jupiter.api.Assertions.assertEquals
import org.junit.jupiter.api.Nested
import org.junit.jupiter.api.extension.ExtensionContext
import org.junit.jupiter.params.ParameterizedTest
import org.junit.jupiter.params.provider.Arguments
import org.junit.jupiter.params.provider.ArgumentsProvider
import org.junit.jupiter.params.provider.ArgumentsSource
import java.util.stream.Stream

internal class Pattern132Test {

    class InputArgumentsProvider : ArgumentsProvider {

        override fun provideArguments(context: ExtensionContext): Stream<out Arguments> = Stream.of(
            Arguments.of(
                intArrayOf(1, 2, 3, 4),
                false
            ),
            Arguments.of(
                intArrayOf(3, 1, 4, 2),
                true
            ),
            Arguments.of(
                intArrayOf(-1, 3, 2, 0),
                true
            )
        )
    }

    @Nested
    inner class Pattern132BruteForceInCubicTimeTest {

        @ParameterizedTest
        @ArgumentsSource(InputArgumentsProvider::class)
        fun `should check if there is a 132 pattern in nums`(nums: IntArray, expected: Boolean) {
            Pattern132BruteForceInCubicTime().test(nums, expected)
        }
    }

    @Nested
    inner class Pattern132BruteForceInQuadraticTimeTest {

        @ParameterizedTest
        @ArgumentsSource(InputArgumentsProvider::class)
        fun `should check if there is a 132 pattern in nums`(nums: IntArray, expected: Boolean) {
            Pattern132BruteForceInQuadraticTime().test(nums, expected)
        }
    }

    private fun Pattern132.test(nums: IntArray, expected: Boolean) {
        val actual = find132pattern(nums)
        assertEquals(expected, actual)
    }
}